...
- **AI 自动回复** - 智能自动响应终端提示，内置安全控制
- **工作流日志** - 会话级事件日志，用于调试和审计 AI 操作
- **邮件通知** - 会话需要输入或任务完成时发送提醒
- **OSC 52 剪贴板** - 通过 tmux 将选中文本直接复制到系统剪贴板
- **移动端友好** - 响应式 UI，支持工作流日志、AI 状态显示和触摸操作
- **安全访问** - 基于 PIN 码认证（6-12 位字母数字）和 JWT 令牌
...
└── scripts/
    ├── install.sh           # 一键安装脚本
    ├── build.sh             # 构建脚本
    └── idea-context-server-1.0.0.zip  # IntelliJ IDEA MCP 插件
```
...
2. 选择 `scripts/idea-context-server-1.0.0.zip`
3. 重启 IDEA

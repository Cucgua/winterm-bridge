...
| Trellis browser | `getSessionTrellisSummary`, `getSessionTrellisTask`, `getSessionTrellisSpec`, `getSessionTrellisSource` | API and translations present; no new client surface | Terminal | Right-side overlay drawer above xterm | Later | Rebuild into client style instead of copying old web panel layout directly. |

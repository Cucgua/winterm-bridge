...
export function ActivityBar({ activeSection, aiEnabled, hasToken, serverName, onSectionChange, onLogout }: Props) {
  ...
  return (
    <div className="w-12 bg-sidebar flex flex-col items-center py-3 gap-1 border-r border-white/10 shrink-0">
      {navItems.map(item => {
        ...
        return (
          <button
            ...
            className={`relative flex items-center justify-center w-10 h-10 rounded-lg transition-colors ${
              isActive
                ? 'text-accent'
                : 'text-text-tertiary/30 hover:text-text-primary/95 hover:bg-white/5'
            }`}
      ...
      <button
        className="flex items-center justify-center w-10 h-10 rounded-lg text-text-tertiary/30 hover:text-error hover:bg-white/5 transition-colors mt-1"
        onClick={onLogout}
        title={serverName ? `${serverName} — click to logout` : 'Logout'}
      >

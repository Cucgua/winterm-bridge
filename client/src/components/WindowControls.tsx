    ...
    <button
      ...
      className={`flex h-8 w-8 items-center justify-center rounded-lg border border-transparent text-text-secondary/55 transition-colors ${
        danger
          ...
          : 'hover:border-theme-border/10 hover:bg-surface-highlight/55 hover:text-text-primary/95'
      }`}
      title={title}
...
export function WindowControls() {
  ...
  return (
    <div className="flex flex-shrink-0 items-center gap-1">
      <WindowButton title={t('window_minimize')} action="minimize">
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18h12" />
        </svg>
      </WindowButton>
      <WindowButton title={t('window_maximize')} action="maximize">
        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <rect x="6" y="6" width="12" height="12" rx="1.5" />
        </svg>
      </WindowButton>
      <WindowButton title={t('window_close')} action="close" danger>
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
